    ...
    <div className="bg-[#F0F4FA] min-h-screen font-sans flex flex-col">
      ...
      <section className="py-16 md:py-24 max-w-[1440px] mx-auto px-6 md:px-10 lg:px-16 w-full">
        <div className="flex flex-col items-center mb-16 text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-rose-100/50 border border-rose-200 mb-6">
            <Newspaper className="w-4 h-4 text-rose-600" />
            <span className="text-[12px] font-bold text-rose-700 uppercase tracking-wider">Latest Articles</span>
          </div>
          <h2 
          ...
          {posts.map((post) => (
            <article key={post.id} className="bg-white rounded-3xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col">
              <div className="relative h-60 overflow-hidden bg-slate-100">
                ...
                />
                <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-[11px] font-bold text-rose-600 uppercase tracking-wider shadow-sm">
                  {post.category}
                </div>
              </div>
              <div className="p-8 flex flex-col flex-grow">
                <div className="flex items-center space-x-4 text-[13px] text-slate-500 font-medium mb-4">
                  ...
                  <div className="flex items-center">
                    <User className="w-4 h-4 mr-1.5 text-rose-500" />
                    {post.author}
                  </div>
                </div>
                <h3 
                  className="text-[22px] font-bold text-[#0B1F3A] mb-4 group-hover:text-rose-600 transition-colors leading-snug"
                  style={{ fontFamily: "var(--font-montserrat, sans-serif)" }}
                >
                ...
                <Link 
                  href={`/blog/${post.slug}`}
                  className="inline-flex items-center text-[14px] text-rose-600 font-bold hover:text-rose-800 transition-colors"
                >
                  Read Article
        ...
        {posts.length === 0 && (
          <div className="text-center py-12">
            <p className="text-slate-500 text-[16px] font-medium">No blog posts found. Check back later!</p>
          </div>
        )}
      </section>
